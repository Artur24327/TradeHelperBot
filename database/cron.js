            ...
            dbSignals.forEach(element => {
                if(element.triggervalue == '>'){
                    if(binanceTickers[element.symbol] > element.price){
                        bot.signalAlert(element.idchat, `Signal ${element.symbol} on price ${element.price}`);
                        bdSignal.signalController.deleteSignal(element.idchat, element.symbol, element.price);
                    } 
